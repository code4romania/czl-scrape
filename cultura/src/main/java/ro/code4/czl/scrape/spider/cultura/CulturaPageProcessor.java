...
public class CulturaPageProcessor implements PageProcessor {
  ...
  private static final Pattern PATTERN_PROJECT_TYPE = Pattern
      .compile("(?<projectType>hotarare|lege|oug|hg|ordin|og|om)", Pattern.CASE_INSENSITIVE);
  ...
  public void process(Page page) {
    ...
    if (page.getUrl().toString().equalsIgnoreCase("http://www.cultura.ro/proiecte-acte-normative")) {
      ...
      String cleanedTitle = StringUtils.stripAccents(proposalPageTitle);
      Matcher projectTypeMatcher = PATTERN_PROJECT_TYPE.matcher(cleanedTitle);
      ...
      if (projectTypeLabel != null) {
        ...
        logger.info("Parsed projectType={} projectTypeLabel={}", type, projectTypeLabel);
      } else {
        logger.warn("Don't know how to parse title={} url={}", proposalPageTitle, page.getUrl());
        return;
      }
