...
public class ScraperComunicatii {
	...
	private static final String DATE_PUBLICARE_REGEX = "(publicat).+"
			+ DATE_REGEX;
	private static final String DATE_FEEDBACK_REGEX = "(comentarii).+"
			+ DATE_REGEX;
	...
	public static void main(String[] args) {
		...
		try {
			...
			// document =
			// Jsoup.parse(ScraperComunicatii.class.getResourceAsStream("/Proiecte de Acte Normative _ Comunicatii.gov.ro.html"),
			// "UTF-8","");
		} catch (IOException e) {
			e.printStackTrace();
		}
		...
		// pimul div
		//Elements div_wpb_wrappers = document.select("div.wpb_column.vc_column_container.vc_col-sm-6:first-of-type");
		
		Elements div_wpb_wrappers = document.select("div.wpb_content_element")
				.select("div.wpb_wrapper");
										...
										// System.out.println(item);
											if ("hr".equals(item.nodeName())) {
															...
															gEl.childNodes()
																	.forEach(
																			cNode -> {
																				...
																				case "a": {
																					description
																							.append(getTextFromNode(cNode));
																					cBean.addDocument(
																							cNode.attr("href"),
																							getTextFromNode(cNode));
																				}
																				}
												...
												cBean.setDescription(description
														.toString());
												cBean.setTitle(getTextFromNode(groupedElements
														.get(0)));

												extractDate(cBean);
												String feedback = extractFeedbackDate(
														cBean.getDescription(),
														DATE_FEEDBACK_REGEX);
												String start = extractFeedbackDate(
														cBean.getDescription(),
														DATE_PUBLICARE_REGEX);

												cBean.setFeedback_days(calculateFeedbackDays(
														start, feedback));
												cBean.setType(deductType(description
														.toString()));
												// System.out.println(cBean.toJson());

												cBean.setIdentifier(new UID()
														.toString()
														+ "1:"
														+ cBean.getType()
														+ ":"
												...
												groupedElements.clear();
												 ...
												 if (StringUtils.isNotBlank (item.toString ())) {
						                              /*  if (!item.toString ().contains ("<strong>")) {
		...
		outputList.forEach (el -> {
            if (!el.title.contains ("Propunerea de D")
                    && !el.title.contains ("Propunerea de Regulament")
                    && !el.title.contains ("Comunicarea Comisiei")
                    && !(el.date == null)) {
	...
	private static String extractFeedbackDate(String description,
			String dateFeedbackRegex) {
	...
	private static String getTextFromNode(Node cNode) {
		...
		cNode.childNodes().forEach(node -> {
			...
			case "#text":
				...
				break;
			case "a":
				text.append(getTextFromNode(node));
				break;
			}
		});
	...
	private static void selectLinks(Document document) {
		...
		for (Element link : links) {
			...
			link.textNodes().forEach(
					linknode -> System.out.println("link node: "
							+ linknode.toString()));
	...
	private static void extractDocument(Element link, ComunicatiiBean cBean,
			ComunicatiiBean.Document cDoc) {
	...
	public static String calculateFeedbackDays(String dataPublicare,
			String dataFeedback) {
	...
	public static String deductType(String description) {
		...
		if (description.contains("Proiect de Lege")
				|| description.contains("Proiect de lege")) {
			type = type + "LEGE";
		}
		if (description.contains("Ordonanță")
				|| description.contains("Proiect de Ordonanță")
				|| description.contains("Proiect de ordonanță")
				|| description.contains("Ordonanţă")) {
			type = type + "OG";
		}
